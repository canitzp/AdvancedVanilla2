...
public class AVConfig {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.replace(" ", "").equals(lineToChange.replace(" ", ""))) {
                    input += "    # The next Line is modified by AdvancedVanilla2!" + System.lineSeparator() + "    " + replacedLine + System.lineSeparator();
                    newEntry = true;
                    changes++;
                }else if(line.contains("# AdvancedVanilla2 last modified:") && newEntry){
                    input += writeCalender();
                    lastModText = true;
                } else {
    ...
    private static String writeCalender(){
        ...
        input += "# AdvancedVanilla2 last modified: " + year + "/" + month + "/" + day + ", " + hour + ":" + minute;
        return input;
    }
